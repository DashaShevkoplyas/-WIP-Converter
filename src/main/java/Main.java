...
public class Main {
    ...
    private static Optional<File> lastCreatedFileIn(File folder) throws FileNotFoundException {
    ...
    private static boolean hasMP4Format(String fileName) {
        return FilenameUtils.getExtension(fileName).equals("mp4");
    }

    private static long creationTimeInMills(Path path) {
        try {
    ...
    private static Path getDefaultSystemPath(String... customPath) {
